<#
.SYNOPSIS
    Removes (undoes) the DurabilityTweaks behavior pack installed by the companion install script.
#>
...
try {
    Write-Log "Starting DurabilityTweaks uninstall process..."
    ...
    # ---------------------------------------------------------------------------
    Write-Log "Step 3: Removing DurabilityTweaks from world_behavior_packs.json..."
    ...
    if (-not (Test-Path -LiteralPath $worldDir -PathType Container)) {
        ...
        if (-not $malformed) {
            ...
            if (-not $toRemove) {
                Write-Log "DurabilityTweaks UUID was not registered in this world. No JSON changes needed." -Level "OK"
            } else {
                # Rebuild the list without the DurabilityTweaks entry/entries
    ...
    # ---------------------------------------------------------------------------
    Write-Log "Step 4: Removing DurabilityTweaks behavior pack directory..."
    ...
    Write-Log "IMPORTANT: Restart your Bedrock server so the changes take effect."
    Write-Log "Vanilla durability will be restored, but previously modified items will keep their stats."
    Write-Log "============================================================="
