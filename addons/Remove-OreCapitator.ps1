<#
.SYNOPSIS
    Removes (undoes) the Orecapitator behavior pack installed by the companion install script.
#>
...
try {
    Write-Log "Starting Orecapitator uninstall process..."
    ...
    # ---------------------------------------------------------------------------
    Write-Log "Step 3: Removing Orecapitator from world_behavior_packs.json..."
    ...
    if (-not (Test-Path -LiteralPath $worldDir -PathType Container)) {
        ...
        if (-not $malformed) {
            ...
            if (-not $toRemove) {
                Write-Log "Orecapitator UUID was not registered in this world. No JSON changes needed." -Level "OK"
            } else {
                # Rebuild the list without the Orecapitator entry/entries
    ...
    # ---------------------------------------------------------------------------
    Write-Log "Step 4: Removing Orecapitator behavior pack directory..."
    ...
    Write-Log "Ore vein mining will be disabled after the restart."
    Write-Log "============================================================="
