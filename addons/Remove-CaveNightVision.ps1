<#
.SYNOPSIS
    Removes (undoes) the CaveVision behavior pack installed by the companion install script.
#>
...
try {
    Write-Log "Starting CaveVision uninstall process..."
    ...
    # ---------------------------------------------------------------------------
    Write-Log "Step 3: Removing CaveVision from world_behavior_packs.json..."
    ...
    if (-not (Test-Path -LiteralPath $worldDir -PathType Container)) {
        ...
        if (-not $malformed) {
            ...
            if (-not $toRemove) {
                Write-Log "CaveVision UUID was not registered in this world. No JSON changes needed." -Level "OK"
            } else {
                # Rebuild the list without the CaveVision entry/entries
    ...
    # ---------------------------------------------------------------------------
    Write-Log "Step 4: Removing CaveVision behavior pack directory..."
    ...
    Write-Log "IMPORTANT: Restart your Bedrock server so the script is fully unloaded."
    Write-Log "Players will lose their permanent Night Vision after the restart."
    Write-Log "============================================================="
